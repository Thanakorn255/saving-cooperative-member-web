      ...
      <div class="mt-8 h-full overflow-x-scroll xl:overflow-hidden">
        <table {...getTableProps()} className="w-full">
          ...
          <tbody {...getTableBodyProps()} >
            {page.map((row, index) => {
              prepareRow(row);
              console.log("AccountNo",row.allCells[0].value);
              return (
                <tr {...row.getRowProps()} key={index} className={`${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}`}>
                  ...
                  {row.cells.map((cell, index) => {
                    ...
                    if (cell.column.Header === "AccountNo") {
                      data = (
                        <p className="text-sm font-bold text-navy-700 dark:text-white">
                          {cell.value}
                        </p>
                      );
                    } else if (cell.column.Header === "AccountType") {
                      data = (
                        <p className="text-sm font-bold text-navy-700 dark:text-white">
                          {cell.value}
                        </p>
                    ...
                    return (
                      ...
                      >
                       ...
                       <a href={`/admin/deposit-trans/${row.allCells[0].value}/${row.allCells[1].value}`}> {data}</a>
                      </td>
                    );
                  })}
                </tr>
